...
async function gql(query: string, variables: any = {}) {
  const r = await fetch(CONTENT_URL, { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-role': 'specialist' }, body: JSON.stringify({ query, variables }) });
...
async function gql2(query: string, variables: any = {}) {
  const r = await fetch(CONTENT_URL, { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-role': 'specialist', 'x-user-id': 'me-specialist' }, body: JSON.stringify({ query, variables }) });
  const j = await r.json();
  if (j.errors) throw new Error(j.errors[0]?.message || 'error');
  return j.data;
}
...
export default function Page() {
  ...
  const addTo = (section: 'warmup' | 'main' | 'cooldown', v: any) => {
    ...
    copy[0][section] = [...copy[0][section], v];
    setDays(copy);
  };
  ...
  async function assign() {
    ...
    try {
      ...
      await gql2(`mutation($pid:String!,$uid:String!,$sd:String!,$spw:Int){ assignPlanToUser(planId:$pid, userId:$uid, startDate:$sd, sessionsPerWeek:$spw) }`, { pid: planId, uid: userId, sd: startDate, spw });
      setStatus('Assigned ✓');
    } catch (e: any) {
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <div style={{ display: 'grid', gap: 12, gridTemplateColumns: '320px 1fr 1fr' }}>
        <div>
          <AnatomyBoard sex={sex} onPick={(code: string) => setSel((s) => Array.from(new Set([...(s || []), code])))} />
        </div>
        <div>
