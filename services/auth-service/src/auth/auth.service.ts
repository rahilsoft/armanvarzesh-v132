...
export class AuthService {
  ...
  async register(email: string, password: string, ctx: SessionContext = {}): Promise<AuthResult> {
  ...
  async issueTokens(user: { id: string; email: string }, ctx: SessionContext = {}): Promise<AuthResult> {
    const jti = randomUUID();
    const claims: AccessTokenClaims = { sub: user.id, email: user.email, jti };
