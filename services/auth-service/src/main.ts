...
app && applyBasicHardening(app);
  ...
  if (process.env.ENABLE_OTEL === 'true') { try { setupObservability(); } catch (e) { new Logger('AuthBootstrap').error('OTEL init failed', e); } }
const swaggerCfg = new DocumentBuilder().setTitle('ArmanFit').setDescription('API').setVersion('1.0').addBearerAuth().build();
  ...
  setGlobalDispatcher(new Agent({ keepAliveTimeout: 10_000, keepAliveMaxTimeout: 10_000 }));
...
if (http && typeof http.get === 'function') {
  ...
  await bootstrapSecurityAndObservability(app, 'auth-service')
  ...
  new Logger('AuthBootstrap').log(`Auth service listening on port ${port}`);
}
