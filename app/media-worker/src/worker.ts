...
// Fixed: removed duplicate options object
new Worker('media:transcode', transcode, {
  connection,
  concurrency: parseInt(process.env.WORKER_CONCURRENCY || '4') || env.concurrency
});

new Worker('media:image', processImage, {
  connection,
  concurrency: parseInt(process.env.WORKER_CONCURRENCY || '4') || env.concurrency
});
