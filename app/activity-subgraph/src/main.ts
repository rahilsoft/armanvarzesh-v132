...
app && applyBasicHardening(app);
  app.get('/health', (_req,res)=> res.json({ok:true, service:'activity-subgraph'}));
  const server = new ApolloServer({ schema: buildSubgraphSchema([{ typeDefs, resolvers }]) }); await server.start();
  app.use('/graphql', expressMiddleware(server) as any); const httpServer = http.createServer(app); const port = Number(process.env.PORT || 4005);
  httpServer.listen(port, ()=> console.log(`activity-subgraph up on :${port}`)); }
